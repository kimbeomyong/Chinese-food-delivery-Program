...
import Cart.Customer;

public class DeliveryMain {

	static final int NUM_MENU = 3;// 메뉴 갯수
	static final int NUM_ITEM = 5;// 메뉴 정보 갯수
	static CartItem[] cartItem = new CartItem[NUM_MENU];// 장바구니
	static int cartCount = 0; // 장바구니 도서 갯수
	...
	public static void main(String[] args) {
		...
		String adress; // 주소
		int number;// 메뉴 번호 선택
		String[][] menu = new String[NUM_MENU][NUM_ITEM];// 메뉴 정보 목록
		...
		adress = scan.next();// 주소를 문자열로 입력받음
	...
	public static void menuInformation(String name, int phoneNumber, String adress) {
		Customer ct = new Customer(name, phoneNumber, adress);
		System.out.println("현재 고객 정보");
		System.out.println("이름 : " + ct.getName() + ", 연락처 : " + ct.getPhone() + ",주소 : " + ct.getAddress());
	}
		...
		while (!quit) {
			...
			if (flag) {
				...
				if (inputStr.toUpperCase().equals("Y") || inputStr.toUpperCase().equals("y")) {
					...
					// 장바구니에 담기
					if (!isCartInMenu(menu[num][0])) {
						cartItem[cartCount++] = new CartItem(menu[num]);
					}
				}
				quit = true;
			} else {
